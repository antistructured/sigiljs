  ...
  switch (ast.kind) {
    ...
    case 'primitive_union': {
      ...
      const allSimple = names.every(
        (n) => n === 'string' || n === 'number' || n === 'boolean' ||
               n === 'symbol' || n === 'bigint' || n === 'undefined'
      )
      return allSimple
    ...
    case 'object': {
      ...
      const req = hints
        ? hints.requiredKeys.map((k) => ({ key: k, check: build(properties.find((p) => p.key === k).value) }))
        : properties.filter((p) => !p.optional).map((p) => ({ key: p.key, check: build(p.value) }))
      const opt = hints
        ? hints.optionalKeys.map((k) => ({ key: k, check: build(properties.find((p) => p.key === k).value) }))
        : properties.filter((p) => p.optional).map((p) => ({ key: p.key, check: build(p.value) }))
      ...
      return (v, opts) => {
        ...
        for (let i = 0; i < optLen; i++) {
          const p = opt[i]
          if (p.key in v && v[p.key] !== undefined && !p.check(v[p.key], opts)) return false
        }
        return true
      }
    }
