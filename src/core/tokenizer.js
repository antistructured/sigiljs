...
export function tokenize(input) {
  ...
  while (pos < len) {
    ...
    if (ch === '"' || ch === "'") {
      const quote = advance()
      const p = pos - 1, l = line, c = column - 1
      const chars = []
      let closed = false
      while (pos < len) {
        const next = advance()
      ...
      tokens.push({ kind: 'string literal', value: chars.join(''), start: p, end: pos, line: l, column: c })
      continue
    }
    ...
    if (ch >= '0' && ch <= '9' || ch === '-') {
      ...
      tokens.push({ kind: 'number literal', value: Number(chars.join('')), start: p, end: pos, line: l, column: c })
      continue
    }
