...
describe('validate & assert', () => {
  ...
  it('assert returns true on success', () => {
    const schema = ast('string');
    expect(assert(schema, 'hello')).toBe(true);
  });
  ...
  it('error has the canonical shape', () => {
    ...
    let err;
    try { assert(schema, 'oops'); } catch (e) { err = e; }
  ...
  it('toJSON() returns the canonical structured shape', () => {
    const schema = ast('number');
    let err;
    try { assert(schema, 'hello'); } catch (e) { err = e; }
  ...
  it('JSON.stringify(err) produces the canonical shape', () => {
    ...
    let err;
    try { assert(schema, 42); } catch (e) { err = e; }
  ...
  it('primitive: reports correct expected/actual', () => {
    let err;
    try { assert(ast('string'), 42); } catch (e) { err = e; }
  ...
  it('primitive: message is human-readable', () => {
    let err;
    try { assert(ast('number'), 'oops'); } catch (e) { err = e; }
  ...
  it('object: reports path to failing property', () => {
    const schema = ast('{ a: string }');
    let err;
    try { assert(schema, { a: 42 }); } catch (e) { err = e; }
  ...
  it('object: message includes property name', () => {
    ...
    let err;
    try { assert(schema, { age: 'thirty' }); } catch (e) { err = e; }

    expect(err.message).toBe('Expected property "age" to be number, got string');
  });
  ...
  it('object: matches the target error shape from spec', () => {
    ...
    let err;
    try { assert(schema, { user: { age: 'thirty' } }); } catch (e) { err = e; }
  ...
  it('deep: 3-level nesting produces full path', () => {
    ...
    let err;
    try { assert(schema, { a: { b: { c: 42 } } }); } catch (e) { err = e; }
  ...
  it('deep: assert for existing test (profile.email[0])', () => {
    ...
    let err;
    try { assert(schema, { profile: { email: [42] } }); } catch (e) { err = e; }
  ...
  it('array: reports path with index to failing item', () => {
    const schema = ast('number[]');
    let err;
    try { assert(schema, [1, 2, 'bad', 4]); } catch (e) { err = e; }
  ...
  it('array: message includes item index', () => {
    const schema = ast('string[]');
    let err;
    try { assert(schema, ['ok', 42]); } catch (e) { err = e; }
  ...
  it('array inside object: full path includes key and index', () => {
    ...
    let err;
    try { assert(schema, { tags: ['news', 99] }); } catch (e) { err = e; }
  ...
  it('missing required property: path points to the key', () => {
    ...
    let err;
    try { assert(schema, { name: 'D' }); } catch (e) { err = e; }
  ...
  it('missing required property: message names the property and expected type', () => {
    ...
    let err;
    try { assert(schema, {}); } catch (e) { err = e; }

    expect(err.message).toBe('Missing required property "email" (expected string)');
  });
  ...
  it('missing required property: complex expected type in message', () => {
    ...
    expect(err.expected).toBe('string[]');
    expect(err.message).toBe('Missing required property "tags" (expected string[])');
  });
  ...
  it('exact: reports unexpected extra property', () => {
    ...
    let err;
    try { User.assert({ name: 'D', admin: true }); } catch (e) { err = e; }
  ...
  it('primitive union: reports all expected types', () => {
    ...
    let err;
    try { assert(schema, true); } catch (e) { err = e; }
  ...
  it('complex union: surfaces deepest sub-path failure', () => {
    ...
    let err;
    try { assert(schema, { name: 42 }); } catch (e) { err = e; }
  ...
  it('identifier: resolves named sigil for error reporting', () => {
    ...
    let err;
    try { User.assert({ email: 42 }); } catch (e) { err = e; }
  ...
  it('identifier: reports missing ref gracefully (re-wrapped as SigilValidationError)', () => {
    ...
    let err;
    try { assert(schema, { ref: 1 }); } catch (e) { err = e; }
  ...
  it('Sigil.assert() returns true on valid data', () => {
    const User = Sigil`{ name: string }`;
    expect(User.assert({ name: 'D' })).toBe(true);
  });
  ...
  it('Sigil.assert() error has correct path and types', () => {
    ...
    let err;
    try { Post.assert({ title: 'Hi', views: 'many' }); } catch (e) { err = e; }

    expect(err.path).toEqual(['views']);
    expect(err.expected).toBe('number');
    expect(err.actual).toBe('string');
    expect(err.message).toBe('Expected property "views" to be number, got string');
  });
});
